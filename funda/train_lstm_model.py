...
def train_lstm_model(X_order_footprint, X_scalar, y, input_size, output_size, epochs=100, batch_size=32, fine_tune=False, use_cnn=False, price_levels=10, channels=2):
    ...
    if fine_tune:
        ...
        model_path = os.path.join(model_dir, 'lstm_daily_model.pt' if not use_cnn else 'lstm_1minute_model.pt')
        if os.path.exists(model_path):
            model.load_state_dict(torch.load(model_path))
        else:
            logging.warning(f"Model file {model_path} not found for fine-tuning. Starting from scratch.")
    
    criterion = trend_loss
    optimizer = torch.optim.AdamW(model.parameters(), lr=5e-5 if fine_tune else 1e-4)  # AdamW and lower LR
    ...
    if use_cnn:
        train_dataset = TensorDataset(X_order_footprint[train_idx], X_scalar[train_idx], y_tensor[train_idx])
        val_dataset = TensorDataset(X_order_footprint[val_idx], X_scalar[val_idx], y_tensor[val_idx])
    else:
        train_dataset = TensorDataset(X_scalar[train_idx], y_tensor[train_idx])
        val_dataset = TensorDataset(X_scalar[val_idx], y_tensor[val_idx])
    ...
    for epoch in range(epochs):
        ...
        logging.info(f"Epoch {epoch+1}/{epochs}, Train Loss: {avg_loss:.6f}, Val Loss: {avg_val_loss:.6f}, LR: {optimizer.param_groups[0]['lr']}")
        ...
        if avg_val_loss < best_loss:
            ...
            model_path = os.path.join(model_dir, 'lstm_daily_model.pt' if not use_cnn else 'lstm_1minute_model.pt')
            try:
                logging.info(f"Saving model to {model_path} (val loss: {avg_val_loss:.6f})")
    ...
    model_path = os.path.join(model_dir, 'lstm_daily_model.pt' if not use_cnn else 'lstm_1minute_model.pt')
    try:
        logging.info(f"Saving final model to {model_path} (best val loss: {best_loss:.6f})")
        torch.save(model.state_dict(), model_path)
...
if __name__ == "__main__":
    ...
    logging.info("Testing model training with minimal data...")
    
    # Create minimal test data
    np.random.seed(42)
    test_X = np.random.randn(100, 60, 14)  # 100 samples, 60 time steps, 14 features
    test_y = np.random.randn(100, 30)      # 100 samples, 30 predictions
    
    try:
        test_model = train_lstm_model(
            X_order_footprint=None, 
            X_scalar=test_X, 
            y=test_y, 
            input_size=14, 
            output_size=30, 
        ...
        current_dir = os.path.dirname(os.path.abspath(__file__))
        model_path = os.path.join(current_dir, "model", "lstm_daily_model.pt")
        if os.path.exists(model_path):
            logging.info(f"Model file created successfully at: {model_path}")
            file_size = os.path.getsize(model_path)
            logging.info(f"Model file size: {file_size} bytes")
        else:
            logging.error("Model file was not created!")
            
    except Exception as e:
        logging.error(f"Test training failed: {e}")
        import traceback
        traceback.print_exc()
    
    # Run the full training if test passes
